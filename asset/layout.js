...
const headerHTML = `
    ...
    const footerHTML = `
        <div class="footer-links">
            <a href="../pages/about.html">關於我們</a> <span>|</span>
...
function handleAction(msg, event) {
    ...
    setTimeout(() => { 
        ...
        // --- 新增這段判斷 ---
        // 如果訊息包含 "登入成功"，就跳轉到會員頁面
        if (msg.includes("登入成功")) {
            // 判斷目前層級，如果在 pages 資料夾內就直接連，如果在根目錄就要加 pages/
...
function checkEmptyCart() {
    const items = document.querySelectorAll('.item-card');
    const cartItemsContainer = document.getElementById('cartItems'); // 商品列表區塊
    const summaryPanel = document.querySelector('.summary-panel');   // 右邊結帳區塊
        ...
        if(cartItemsContainer) {
            cartItemsContainer.innerHTML = `
                <div style="text-align:center; padding: 60px 20px;">
                    <h3 style="color:#666; margin-bottom: 20px;">購物車空空如也</h3>
                    <p style="color:#999; margin-bottom: 30px;">趕快去選購喜歡的商品吧！</p>
                    <a href="../index.html" style="background:#ff4d4f; color:white; padding:10px 30px; border-radius:8px; text-decoration:none;">
                        去逛逛
                    </a>
                </div>
            `;
